...
function createExecutor(
  ...
  return async (args: unknown, exec: ToolExecution) => {
    ...
    // 2026-07-28 MRTR: the server asks for mid-call input. This bridge has no
    // user-interaction channel, so refuse loudly instead of rendering nothing.
    if ((result as { resultType?: unknown }).resultType === 'input_required') {
      throw new Error(
        `Tool "${rawName}" requires interactive input mid-call (MRTR), which this bridge does not fulfill`,
      )
    }
